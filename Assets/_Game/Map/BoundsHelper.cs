...
public class BoundsHelper : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        {
            tilemap.SetTile(new Vector3Int(x, parent.mapHeightOffset - parent.mapHeight + 1, 0), previewTile);
            tilemap.SetTile(new Vector3Int(x, parent.mapHeightOffset, 0), previewTile);
        }
        for (int y = parent.mapHeightOffset - parent.mapHeight + 1; y <= parent.mapHeightOffset; y++)
        {
            tilemap.SetTile(new Vector3Int(parent.mapWidthOffset, y, 0), previewTile);
            tilemap.SetTile(new Vector3Int(parent.mapWidth + parent.mapWidthOffset - 1, y, 0), previewTile);
        }
